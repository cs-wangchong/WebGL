...
var OBJDoc = function(fileName) {
  ...
  this.vNormal = new Array(0);    // 增加一个用来存对象上每个点的法向的数组
...
OBJDoc.prototype.parse = function(fileString, scale, reverse) {
  ...
  while ((line = lines[index++]) != null) {
    ...
    case 'v':   // Read vertex
      ...
      this.vNormal.push(new Vector3([0,0,0]));
      continue; // Go to the next line
    case 'vn':   // Read normal
    ...
    case 'usemtl': // Read Material name
      ...
      continue; // Go to the next line
    case 'f': // Read face
      var face = this.parseFace(sp, currentMaterialName, this.vertices, this.vNormal, reverse);
...
OBJDoc.prototype.parseFace = function(sp, materialName, vertices, vNormal, reverse) {
  ...
  // 为了达到phong shader的效果，将每个点的法向量在每个面上做累加，之后再标准化
  for(var i = 0; i<face.numIndices; i++) {
    var temp = new Vector3(normal);
    vNormal[face.vIndices[i]] = VectorAdd(vNormal[face.vIndices[i]], temp);
  }
...
OBJDoc.prototype.getDrawingInfo = function() {
  ...
  // 将所有点的法向量标准化
  for(var i = 0; i<this.vNormal.length; i++) {
    this.vNormal[i].normalize();
  }
